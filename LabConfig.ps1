...
$LabConfig = @{
    ...
    DomainNetbiosName = 'AOAGLAB'
    DomainName = 'aoaglab.vlack.com'
    Internet = $True
    ServerISOFolder = "$WSLabDirectory\ISO"
    ServerMSUsFolder = "$WSLabDirectory\ServerMSUs"
    # ClientMSUsFolder = "$WSLabDirectory\ClientMSUs"
    DCVMProcessorCount = 1
...
foreach ($idx in 1..2) {
    $LABConfig.VMs += @{
        ...
        DSCMode = 'Pull'
        # Config = list of configurations to pull from the server
